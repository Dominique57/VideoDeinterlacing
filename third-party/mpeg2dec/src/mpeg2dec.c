...
static void decode_mpeg2 (uint8_t * current, uint8_t * end)
{
	...
	case STATE_SEQUENCE:
	    ...
	    /* might set fbufs */
        {
            ...
            float fps = 27000000.0f / seq->frame_period;
            int isProgressiveSeq = seq->flags & SEQ_FLAG_PROGRESSIVE_SEQUENCE;
            fprintf(stdout, "SEQUENCE fps:%.2f progressiveSeq:%d\n", fps, isProgressiveSeq);
        }
	...
	case STATE_PICTURE:
	    ...
	    /* might set fbuf */
        {
            ...
            int isProgressive = (pic->flags & PIC_FLAG_PROGRESSIVE_FRAME) != 0;
            int isTopFieldFirst = (pic->flags & PIC_FLAG_TOP_FIELD_FIRST) != 0;
            int isRepeatFirstField = (pic->flags & PIC_FLAG_REPEAT_FIRST_FIELD) != 0;
            if (info->display_fbuf && output->draw)
                fprintf(stdout,
                        "IMAGE %d.pgm isProgressive:%d isTopFieldFirst:%d isRepeatFirstField:%d\n",
                        ((pgm_instance_t *)output)->framenum, isProgressive, isTopFieldFirst, isRepeatFirstField
                );
        }
	    if (output->set_fbuf) {
		uint8_t * buf[3];
